$.fn.extend({
	...
	lunbo:function(e){
		...
		//初始位置
		for(var i=0;i<lunboCount;i++){
			$('.lunbo .one').eq(i).css({'left':lunboWidth*i});
			
			//点点初始化
			if(i==0){
				$('.pageOne').append('<span class="active"></span>')
			}else{
				$('.pageOne').append('<span></span>')
			}
		}
	...
	slideTimes:function(times){
		...
		//轮播后位置
		for(var a=0;a<lunboCount;a++){
			var _left=lunboWidth*a-lunboWidth*times;
			$('.lunbo .one').eq(a).stop().animate({'left':_left});
		}
	},
	//文字闪烁 参数className为类名
	textFlicke:function (className) {
		var i = 0;
		setInterval(function() {
			var count = $('.' + className + ' label').length;
			if (i >= count) {
				i = 0;
			}
			$('.' + className + ' label').removeClass('active');
			$('.' + className + ' label').eq(i).addClass('active');
			i = i + 1;
		}, 500);
	},
	//---------------------------随机生成滚球----------------------------------------------------
	//通过id列表设置滚动后要选中的数值 idList表示id列表 numList表示num的列表
	rdScrollNum:function(data) {
		var stopTime = data.showTime * 1000;
		var rd = setInterval(function() {
			$.fn.rdByIdList(data.idLists, data.lastNum);
		}, 100);
		setTimeout(function() {
			clearInterval(rd);
			$.fn.selNum(data.idLists, data.numLists);
		}, stopTime);
	},
	//通过id数据为其赋值随机数
	rdByIdList:function(idList, lastNum) {
		var rd = '';
		var idName = '';
	...
	//随机变化后要呈现的具体数字
	selNum:function(idList, numList) {
		for (var i = 0; i < idList.length; i++) {
			document.getElementById(idList[i]).innerHTML = numList[i];
		}
	},
	//---------------------------小球滚动依次跑出---------------------------------------------------
	CirleScroll:function(e) {
			var defaults = {
				'className':'pCircle',                 //类名
				'numList':[03,05,07,08,06,09,15,22,31],//开奖号码
				'cirleWidth': 55,                      //小球宽度
				'spacing':5,                           //小球间距
				'maxLeft': 800,                        //最远小球的left长度
				'speed':0.3                            //速度 单位s
			};
			var data = $.extend({}, defaults, e);
			var length = $('.'+data.className+' label').length;
			var i = length;
			for(var a=0;a<length;a++){
				$('.'+data.className+' label').eq(a).html(data.numList[a]);
			}
			while (i > 0) {
				i -= 1;
				$.fn.circleAction(data.className,i, data.maxLeft, data.cirleWidth,data.spacing,data.speed);
			}
		},
	//小球开始滚动
	circleAction:function(className,num, maxLeft, cirleWidth,spacing,speed) {
		var length = $('.'+className+' label').length;
		var mLeft = maxLeft - (cirleWidth+spacing) * (length - 1 - num);
		var time = (length - num) * speed*1000;
		setTimeout(function() {
			$('.'+className+' label').eq(num).addClass('active');
			$('.'+className+' label').eq(num).css({
				'margin-left': mLeft
			});
		}, time)
	},
	...
	addressInit: function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea) {
		...
		for (var i = 0; i < $.fn.provinceList.length; i++) {
			cmbAddOption(cmbProvince, $.fn.provinceList[i].name, $.fn.provinceList[i]);
		}
